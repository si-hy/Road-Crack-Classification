...
def set_korean_font():
        ...
        if sys.platform == "win32":  # Windows
            ...
            if 'Malgun Gothic' in available_fonts:
                ...
                if os.path.exists(font_path_win):
                    ...
                    logger.exception(f"하이브리드 Copy-Paste 증강 오류: {json_file_orig.name} - {e}")
        
        logger.info(f"하이브리드 Copy-Paste 증강 완료: 증강 {generated_augmented_count}장 생성")
    ...
    def visualize_augmentation_results(self):
        ...
        fig.suptitle("하이브리드 Copy-Paste 데이터 증강 결과 분석", fontsize=20, fontweight='bold', y=0.98)
        ...
        save_path_fig = self.output_dir / f'hybrid_copypaste_summary_{timestamp_str}.png'
        try:
        ...
        logger.info("\n=== 하이브리드 Copy-Paste 최종 통계 요약 (객체 수 기준) ===")
...
if __name__ == '__main__':
    images_dir_path = r""
    labels_dir_path = r""
    output_dir_path_base = r""
    
    class_names_config = ['ac', 'lc', 'pc', 'tc', 'ph'] 
    
    # 🎯 하이브리드 Copy-Paste 핵심 파라미터 설정
    target_total_images_for_experiment = 1021
    num_pastes_range_config = (2, 6)  # 한 번에 더 많은 객체 붙여넣기
    copy_paste_prob_config = 0.9
    chosen_blend_mode = "simple_alpha"     # 블렌딩 모드 선택
    ac_near_ratio_config = 0.6            # AC 근처 배치 비율 (60%)
    side_margin_percent_config = 0.10      # 양옆 10% 제외 (랜덤 배치용)

    logger.info(f"\n\n{'='*20} 하이브리드 Copy-Paste 클래스 불균형 해소 증강 시작 {'='*20}")
    ...
    logger.info(f"AC 근처 배치 비율: {ac_near_ratio_config*100}%")
    logger.info(f"랜덤 배치 비율: {(1-ac_near_ratio_config)*100}%")
    logger.info(f"양옆 제외 비율 (랜덤용): {side_margin_percent_config*100}%")
    ...
    pipeline_single = HybridCopyPasteAugmentation(
        images_dir=images_dir_path, labels_dir=labels_dir_path,
        output_dir=str(output_dir_single_exp), class_names=class_names_config
    )
    
    pipeline_single.augment_dataset_hybrid_copy_paste(
        target_total_images=target_total_images_for_experiment,
        copy_paste_prob=copy_paste_prob_config,
        num_pastes_range=num_pastes_range_config,
        blend_mode=chosen_blend_mode,
        ac_near_ratio=ac_near_ratio_config,
        side_margin_percent=side_margin_percent_config
    )

    logger.info(f"\n\n하이브리드 Copy-Paste 증강 완료. 결과를 '{output_dir_single_exp}' 폴더와 생성된 시각화 파일에서 확인하세요.")
...
class HybridCopyPasteAugmentation:
    ...
    def find_ac_crack_locations(self, label_data):
        """이미지에서 AC 균열의 중심점들을 찾아 반환"""
        ac_centers = []
        shapes = label_data.get('shapes', [])
        
        for shape in shapes:
            if shape.get('label') == 'ac' and shape.get('shape_type') == 'polygon':
                points_list = shape.get('points', [])
                if len(points_list) >= 3:
                    ...
                    center_x = x + w // 2
                    center_y = y + h // 2
                    ac_centers.append((center_x, center_y))
        
        logger.debug(f"발견된 AC 균열 중심점 {len(ac_centers)}개: {ac_centers}")
        return ac_centers

    def generate_positions_around_ac(self, ac_center, proximity_radius, image_shape):
        """AC 중심 주변에 배치 가능한 위치들을 생성"""
        h_bg, w_bg = image_shape[:2]
        ac_x, ac_y = ac_center
        positions = []
        
        # AC 주변 원형 영역에서 여러 위치 생성
        num_candidates = 12  # 360도를 30도씩 나누어 12개 방향
        min_radius = proximity_radius * 0.3  # 최소 거리
        max_radius = proximity_radius
        
        for i in range(num_candidates):
            angle = (2 * math.pi * i) / num_candidates  # 각도 균등 분산
            
            # 반지름을 랜덤하게 설정 (너무 가깝거나 멀지 않게)
            ...
            candidate_x = ac_x + offset_x
            candidate_y = ac_y + offset_y
            
            # 이미지 경계 내에 있는지 확인
            if 0 <= candidate_x < w_bg and 0 <= candidate_y < h_bg:
                positions.append((candidate_x, candidate_y))
        
        logger.debug(f"AC 중심 {ac_center} 주변에 {len(positions)}개 후보 위치 생성")
        return positions

    def copy_paste_near_ac_cracks(self, background_orig, current_shapes, class_weights, 
                                  num_pastes_range=(1, 3), 
                                  blend_mode='simple_alpha',
                                  proximity_radius=200):
        """AC 균열 근처에 다른 균열들을 배치하는 Copy-Paste"""
        if not any(self.class_objects.values()):
            logger.warning("AC 근처 Copy-Paste를 위한 추출된 객체가 없습니다.")
            return background_orig, []

        # 현재 이미지에서 AC 위치 찾기
        label_data = {'shapes': current_shapes}
        ac_locations = self.find_ac_crack_locations(label_data)
        
        if not ac_locations:
            logger.debug("AC 균열이 없어 AC 근처 Copy-Paste를 건너뜁니다.")
            return background_orig, []
        ...
        # 기존 객체들의 바운딩 박스 계산 (겹침 방지용)
        for shape in current_shapes:
            if shape.get('points') and len(shape['points']) >= 3:
                points = np.array(shape['points'], dtype=np.int32)
                x, y, w, h = cv2.boundingRect(points)
                occupied_bboxes.append([x, y, x + w, y + h])

        # 붙여넣을 클래스들 (AC 제외한 다른 균열들)
        other_crack_types = [cn for cn in self.class_names if cn != 'ac' and self.class_objects.get(cn)]
        if not other_crack_types:
            logger.debug("AC 외의 붙여넣을 균열 타입이 없습니다.")
            return output_image, []

        weights_for_choice = [class_weights.get(cn, 1.0) for cn in other_crack_types]
        ...
        # 각 AC 위치별로 객체 배치 시도
        for ac_center in ac_locations:
            if successfully_pasted_count >= num_pastes_actual:
                break
                
            # AC 주변 후보 위치들 생성
            candidate_positions = self.generate_positions_around_ac(ac_center, proximity_radius, output_image.shape)
            
            if not candidate_positions:
                continue

            # AC 근처에 객체 배치 시도
            for _ in range(min(3, num_pastes_actual - successfully_pasted_count)):  # AC당 최대 3개
                try:
                    # 가중치 기반 클래스 선택
                    selected_class = random.choices(other_crack_types, weights=weights_for_choice)[0]
                    if not self.class_objects[selected_class]:
                        continue
                    ...
                    if obj_img_to_paste is None or obj_img_to_paste.size == 0:
                        continue

                    h_obj_orig, w_obj_orig = obj_img_to_paste.shape[:2]

                    # 스케일링과 회전 적용
                    current_scale = random.uniform(0.7, 1.3)
                    current_rotation = random.uniform(-15, 15) if random.random() < 0.4 else 0
                    ...
                    final_obj_img = cv2.warpAffine(obj_img_to_paste, M_transform, (new_obj_w, new_obj_h), 
                                                  flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=(0,0,0))
                    final_obj_mask_binary = cv2.warpAffine(obj_mask_to_paste, M_transform, (new_obj_w, new_obj_h), 
                    ...
                    if random.random() < 0.3: 
                        color_aug = A.Compose([
                            ...
                            A.HueSaturationValue(hue_shift_limit=8, sat_shift_limit=12, val_shift_limit=8, p=1.0)
                        ])
                        final_obj_img = color_aug(image=final_obj_img)['image']

                    # 후보 위치들 중에서 적합한 위치 찾기
                    found_position = False
                    random.shuffle(candidate_positions)  # 위치 순서 랜덤화
                    
                    for candidate_pos in candidate_positions:
                        # 객체가 이미지 경계를 벗어나지 않도록 조정
                        paste_x = max(0, min(candidate_pos[0] - new_obj_w // 2, w_bg - new_obj_w))
                        paste_y = max(0, min(candidate_pos[1] - new_obj_h // 2, h_bg - new_obj_h))
                        
                        if paste_x + new_obj_w > w_bg or paste_y + new_obj_h > h_bg:
                            continue
                            
                        current_bbox_abs = [paste_x, paste_y, paste_x + new_obj_w, paste_y + new_obj_h]
                        
                        # IoU 체크로 겹침 방지
                        if any(self.calculate_iou(current_bbox_abs, occ_bbox) > 0.2 for occ_bbox in occupied_bboxes):
                            continue
                        
                        # 블렌딩 적용
                        output_image, _ = self.blender.blend_object_onto_background(
                            output_image, final_obj_img, final_obj_mask_binary, 
                            None, paste_x, paste_y, new_obj_w, new_obj_h, blend_mode=blend_mode
                        )
                        
                        # 라벨 정보 추가
                        abs_points_for_label = (final_obj_points_relative_transformed + np.array([paste_x, paste_y])).astype(np.int32).tolist()
                    ...
                    logger.warning(f"AC 근처 Copy-Paste 중 오류: {e}")
                    continue

        logger.debug(f"AC 근처에 {successfully_pasted_count}개의 객체 ({blend_mode}) 붙여넣기 완료.")
        return output_image, pasted_shapes_info

    def copy_paste_random_placement(self, background_orig, class_weights, 
                                   num_pastes_range=(1, 3), 
                                   blend_mode='simple_alpha',
                                   side_margin_percent=0.10):
        """전체 영역에서 랜덤하게 Copy-Paste (양옆 마진 제외)"""
        if not any(self.class_objects.values()):
            logger.warning("랜덤 Copy-Paste를 위한 추출된 객체가 없습니다.")
            return background_orig, []
        ...
        # 양옆 마진 계산
        left_margin = int(w_bg * side_margin_percent)   
        right_margin = int(w_bg * side_margin_percent)  
        available_width = w_bg - left_margin - right_margin

        if not class_weights or not any(v > 0 for v in class_weights.values()):
            logger.warning("유효한 클래스 가중치가 없어 랜덤 Copy-Paste를 건너뜁니다.")
            return output_image, []
        ...
        if not classes_with_objects_and_weights:
            logger.warning("붙여넣을 수 있는 객체가 있는 클래스가 없습니다.")
            return output_image, []
        ...
        for _ in range(num_pastes_actual):
            try:
                ...
                current_rotation = random.uniform(-20, 20) if random.random() < 0.4 else 0
                ...
                # 크기 체크 (사용 가능 영역 기준)
                if new_obj_h >= h_bg or new_obj_w >= available_width: continue
                ...
                # paste 위치 계산 (양옆 마진 제외)
                paste_margin = max(5, int(0.01 * min(h_bg, w_bg)))
                min_paste_x = left_margin + paste_margin
                max_paste_x = w_bg - right_margin - new_obj_w - paste_margin
                min_paste_y = paste_margin
                max_paste_y = h_bg - new_obj_h - paste_margin
                
                if max_paste_x <= min_paste_x or max_paste_y <= min_paste_y: continue
                ...
                for _ in range(30): 
                    ...
                    output_image, _ = self.blender.blend_object_onto_background(
                        output_image, final_obj_img, final_obj_mask_binary, 
                        None, current_paste_x, current_paste_y, new_obj_w, new_obj_h, blend_mode=blend_mode
                    )
                ...
                logger.warning(f"랜덤 Copy-Paste 중 오류: {e}")
                continue

        logger.debug(f"랜덤 영역에 {successfully_pasted_count}개의 객체 ({blend_mode}) 붙여넣기 완료.")
        return output_image, pasted_shapes_info

    def hybrid_copy_paste_strategy(self, background_orig, current_shapes, class_weights,
                                  num_pastes_range=(1, 4), 
                                  blend_mode='simple_alpha',
                                  ac_ratio=0.6,
                                  side_margin_percent=0.10):
        """하이브리드 Copy-Paste: AC 근처 60% + 랜덤 배치 40%"""
        output_image = background_orig.copy()
        all_pasted_shapes = []
        
        # 전체 붙여넣기 개수 결정
        min_pastes, max_pastes = num_pastes_range
        total_pastes = random.randint(min_pastes, max_pastes)
        
        # AC 근처와 랜덤 배치 개수 분할
        ac_pastes = int(total_pastes * ac_ratio)
        random_pastes = total_pastes - ac_pastes
        
        logger.debug(f"하이브리드 Copy-Paste: 총 {total_pastes}개 (AC 근처: {ac_pastes}개, 랜덤: {random_pastes}개)")
        
        # 1. AC 근처 Copy-Paste (60%)
        if ac_pastes > 0:
            output_image, ac_pasted_shapes = self.copy_paste_near_ac_cracks(
                output_image, current_shapes, class_weights,
                num_pastes_range=(1, ac_pastes),
                blend_mode=blend_mode,
                proximity_radius=250
            )
            all_pasted_shapes.extend(ac_pasted_shapes)
            # AC 근처에 붙여넣은 객체들도 현재 shapes에 추가 (겹침 방지용)
            current_shapes.extend(ac_pasted_shapes)
        
        # 2. 랜덤 배치 Copy-Paste (40%)
        if random_pastes > 0:
            output_image, random_pasted_shapes = self.copy_paste_random_placement(
                output_image, class_weights,
                num_pastes_range=(1, random_pastes),
                blend_mode=blend_mode,
                side_margin_percent=side_margin_percent
            )
            all_pasted_shapes.extend(random_pasted_shapes)
        
        logger.debug(f"하이브리드 Copy-Paste 총 {len(all_pasted_shapes)}개 객체 배치 완료")
        return output_image, all_pasted_shapes

    def augment_dataset_hybrid_copy_paste(self, target_total_images=2400, 
                                         copy_paste_prob=0.9, 
                                         num_pastes_range=(1,4),
                                         blend_mode='simple_alpha',
                                         ac_near_ratio=0.6,
                                         side_margin_percent=0.10):
        """하이브리드 Copy-Paste 데이터셋 증강"""
        logger.info(f"하이브리드 Copy-Paste 증강 시작: 목표 {target_total_images}장, 블렌드: {blend_mode}")
        logger.info(f"AC 근처 비율: {ac_near_ratio*100}%, 랜덤 비율: {(1-ac_near_ratio)*100}%")
            ...
            for json_file_orig in json_files_original:
                ...
                try:
                    ...
                    if random.random() < copy_paste_prob and any(self.class_objects.values()):
                        augmented_image, new_pasted_shapes = self.hybrid_copy_paste_strategy(
                            augmented_image, current_shapes, class_weights,
                            num_pastes_range=num_pastes_range,
                            blend_mode=blend_mode,
                            ac_ratio=ac_near_ratio,
                            side_margin_percent=side_margin_percent
                        )
                        current_shapes.extend(new_pasted_shapes) 
                        logger.debug(f"하이브리드 Copy-Paste 적용 후 shapes 개수: {len(current_shapes)}")
                    ...
                    aug_base_name = f"{base_name}_hybrid_copypaste_{blend_mode}_{timestamp}_{random.randint(1000,9999)}"
