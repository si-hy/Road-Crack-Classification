...
class OptimizedYOLOAugmentation:
    ...
    def intelligent_copy_paste_with_advanced_blending(self, background_orig, 
        ...
        # --- 🎯 핵심 수정: 전체 이미지 영역에서 Copy-Paste 수행 ---
        logger.info(f"Copy-Paste 영역: 전체 이미지 영역 ({w_bg}x{h_bg})")
        ...
        for _ in range(num_pastes_actual):
            ...
            # --- 🎯 핵심 수정: 전체 이미지 영역에서 paste 위치 계산 ---
            paste_margin = max(5, int(0.01 * min(h_bg, w_bg)))
            
            # X축: 전체 너비에서 배치 가능
            min_paste_x = paste_margin
            max_paste_x = w_bg - new_obj_w - paste_margin
            ...
            if max_paste_x <= min_paste_x or max_paste_y <= min_paste_y: 
                logger.debug(f"객체 크기({new_obj_w}x{new_obj_h})가 너무 커서 배치할 수 없습니다.")
                continue
            
            found_position = False
            for _ in range(30): 
                # 전체 이미지 영역에서 paste 위치 선택
        ...
        logger.debug(f"{successfully_pasted_count}개의 객체(Advanced Blending, 전체 이미지 영역) 붙여넣기 완료 (시도: {num_pastes_actual}개).")
    ...
    def augment_dataset_pipeline(self, pipeline_type="copy_paste_first", 
        ...
        # --- 원본 파일을 복사하지 않고, 0에서부터 시작하여 순수 증강 이미지만 생성 ---
...
if __name__ == '__main__':
    ...
    # 🎯 Copy-Paste를 먼저 수행하도록 파이프라인 타입 설정
    chosen_pipeline_type = "copy_paste_first"  
    chosen_blend_mode = "simple_alpha"     

    logger.info(f"\n\n{'='*20} 전체 이미지 영역 Copy-Paste 증강 시작 {'='*20}")
    ...
    logger.info(f"Copy-Paste 영역: 전체 이미지 (양옆 제한 없음)")
